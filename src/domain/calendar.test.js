...
import { collectUpcomingUpdates, nextObservance, projectShabbat } from "./calendar.js";

describe("calendar domain", () => {
  it("skips past dates and absent months when resolving next observance", async () => {
    const convert = vi.fn(async ({ hy }) => (
      hy === 5786 ? { gy: 2026, gm: 1, gd: 1 } : { gy: 2026, gm: 12, gd: 20 }
    ));
    await expect(nextObservance({ id: "a", hm: "Tevet", hd: 10 }, 5786, convert, "2026-08-30"))
      .resolves.toMatchObject({ iso: "2026-12-20" });

    const missing = vi.fn(async ({ hy }) => {
      if (hy === 5786) throw new Error("invalid");
      return { gy: 2027, gm: 3, gd: 8 };
    });
    await expect(nextObservance({ id: "b", hm: "Adar II", hd: 14 }, 5786, missing, "2026-08-30"))
