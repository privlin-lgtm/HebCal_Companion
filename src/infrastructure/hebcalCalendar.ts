...
export function buildHebcalUrl(
  path: string,
...
export type HebcalCalendar = CalendarPort & { clearCache(): void };
...
export function createHebcalCalendar({
  httpJson,
  apiRoot = HEBCAL_API_ROOT,
  cache = new Map<string, Promise<ConvertResult>>(),
}: {
  httpJson: HttpJson;
  apiRoot?: string;
  cache?: Map<string, Promise<ConvertResult>>;
}): HebcalCalendar {
